...
int server_init(int socket_family, int socket_type, int protocol)
{
	...
	int opt_res = setsockopt(server_fd, protocol,
				 SOL_SOCKET | SO_REUSEADDR | SO_REUSEPORT, &opt,
				 sizeof(opt));
	if (opt_res < 0) {
...
int server_bind(int server_fd, int addr_family, uint32_t addr, uint16_t port)
{
	...
	int bind_res = bind(server_fd, (struct sockaddr *)&server_addr,
			    sizeof(server_addr));
	if (bind_res < 0) {
...
int server_listen(int server_fd, uint32_t limit)
{
	...
	while (1) {
		...
		pthread_t thread;
		pthread_create(&thread, NULL, server_receive,
			       (void *)&client_fd);
...
static void *server_receive(void *client)
{
	...
	int length = 0;
	while (client_fd > 0) {
		ssize_t bytes_received = recv(client_fd, (void *)buffer,
					      sizeof(buffer) / sizeof(char), 0);
		if (bytes_received > 0) {
			regex_t regex;
			...
			if (regexec(&regex, buffer, 2, matches, 0) == 0) {
				...
				if (file != NULL) {
					...
					char *response = malloc(
						strlen(header) + strlen(mime) +
						file_stat.st_size + 30);

					ssize_t bytes_read =
						read(file_fd, file_content,
						     file_stat.st_size);
					...
					memcpy(response + length, file_content,
					       file_stat.st_size);
					...
					const char *response_404 =
						...
						"Content-Length:13\n\n"
						"404 Not Found";
					send(client_fd, response_404,
					     strlen(response_404), 0);
				}
			}
